...
conn = psycopg2.connect(database='crawler', password='snoopy14')
...
'''
alter table youtube_music add column url text;
update youtube_music set url = 'https://www.youtube.com/watch?v=' || (videos->'snippet'->>'resourceId') where coalesce(videos->'snippet'->>'resourceId', '')!='';
'''

with open('youtube_artist_scores.csv', 'r') as rfile, open('youtube_artist_scores_urls.csv', 'w') as wfile:
    rcsv = csv.reader(rfile)
    next(rcsv)
    wcsv = csv.writer(wfile)
    wcsv.writerow(['artist', 'score', 'weighted_score', 'url'])
    for row in rcsv:
        if row[0]:
            cur.execute('select url from %s where artist = %s and url is not null limit 1', [AsIs(table), row[0]])
            u = cur.fetchone()[0]
            wcsv.writerow([row[0], row[1], row[2], u if u else ''])